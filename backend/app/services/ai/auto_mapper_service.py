...
Specialized in automatically mapping Wallcano Tiles and Surfaces Tiles products,
and generating unique, relevant, and consistent Surfaces Tiles product names
using Google Gemini AI based on mapped Wallcano product attributes.
"""
...
class ProductAutoMapperService:
    ...
    def _extract_product_specs(self, walcano_name: str, sku: str = "", category: str = "") -> Dict[str, Any]:
        """Parse structured physical attributes from Wallcano product information."""
                ...
                if gemini_result and gemini_result.get("surfaces_name"):
                    ...
                    if gen_name.lower() not in existing_names_lower:
                        return {
                            ...
                            "reasoning": gemini_result.get("reasoning", "Generated by AI matching Wallcano specifications"),
                            "is_unique": True,
                            "provider": "gemini",
                            "attributes": {
    ...
    async def _generate_name_with_gemini(
        ...
        system_instruction = (
            ...
            "(a luxury architectural B2C tile brand) and Wallcano Tiles (its manufacturing counterpart).\n\n"
            "CRITICAL CONSTRAINTS & SINGLE SOURCE OF TRUTH:\n"
            f"The user has selected ONE specific Wallcano product as the SINGLE SOURCE OF TRUTH: '{selected_name}'.\n"
            "1. You must derive the unique Surfaces Tiles product name EXCLUSIVELY and STRICTLY from this selected Wallcano product's specifications.\n"
            "2. Do NOT use other Wallcano products, alternative matches, or suggested products.\n"
            "3. Do NOT provide alternative product suggestions or names based on other Wallcano products.\n"
            "4. Return EXACTLY ONE unique Surfaces Tiles product name based only on the selected Wallcano product's details.\n\n"
            ...
            "  \"confidence\": 0.98,\n"
            f"  \"reasoning\": \"Derived exclusively from selected Wallcano product '{selected_name}'\"\n"
            "}"
        )

        prompt = (
            f"SELECTED WALLCANO PRODUCT (SINGLE SOURCE OF TRUTH):\n"
            f"- Wallcano Product Name: {specs['walcano_name']}\n"
            f"- SKU: {specs['sku']}\n"
            ...
            f"Remember: Base your name ONLY on this specific selected Wallcano product. Do not suggest or reference any other product. "
    ...
    async def auto_map_single_product(
        ...
        If starting from a Surfaces Tiles product or Wallcano product, automatically:
        1. Maps it to the corresponding Wallcano product.
        2. Generates a unique Surfaces Tiles product name using Gemini AI based on the mapped Wallcano details.
        ...
        # Determine effective Wallcano item
        target_walcano = (walcano_name or "").strip()

        # If only surfaces_name was provided, find corresponding Wallcano product
        ...
        # Generate unique Surfaces Tiles product name based on mapped Wallcano details
        walcano_details = {
